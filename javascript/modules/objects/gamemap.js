...
//INTERNAL FUNCTIONS
async function setLayers(layerURLs) {
  let depth = 0;
  for(let i = 0; i < layers.length; i++) {
    if(!layerURLs[i]) break;
    const map = await Arachnid.Loaders.loadImage(layerURLs[i]);
